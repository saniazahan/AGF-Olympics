...
def _augmented_pair(data_numpy, axis, angle, std):

    C, T, V, M = data_numpy.shape
    #data_numpy = data_numpy.reshape(C, T, V, 1)
    #rotate = Rotate(axis, angle)
    shear = Shear()
    noise = Gaus_noise(mean= 0, std = std)
    
    #data_rot = rotate(data_numpy)
    data_sheared = shear(data_numpy)
    #data_with_added_noise = noise(data_sheared)
    
    #data_with_added_noise = data_with_added_noise.reshape(C, T, V)
    
    return data_sheared#data_with_added_noise
...
def _transform(x, theta):
    #print('original') # N, T, C*V 75
    #print(x.shape)
    x = torch.from_numpy(x)
    C, T, V, M = x.size()
    x = x.permute(3, 1, 0, 2).contiguous().view(M,T,C*V)
    x = x.contiguous().view(x.size()[:2] + (-1, 3))
    #print('new')
    #print(x.shape)
    rot = x.new(1,3).uniform_(-theta, theta)
