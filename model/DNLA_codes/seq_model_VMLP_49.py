...
class MLP(nn.Module):
    def __init__(self, input_size, inchannel, channel_kernel=21, seq_kernel=59, drop_out=0.2, hidden1 = 512, hidden2 = 128, stream='1s'):
        ...
        self.averagepool =  nn.AdaptiveAvgPool2d((inchannel,1))
        #self.NLnet = NLBlockND(in_channels=192, mode='concatenate', dimension=1, bn_layer=True)
        ## End of Vanila block ================================================
    ...
    def forward(self, input_tensor, vid_feat=None):
        ...
        input_tensor = temp#self.act(temp+x3)
        input_tensor.register_hook(self.activations_hook)
        x1 = self.averagepool(input_tensor).squeeze(dim=2)
        #print(x1.shape)
...
#     cd /home/uniwa/students3/students/22905553/linux/phd_codes/Olympics/Olympic_code/
    ...
    seq_model = MLP(input_size=875, inchannel=192, channel_kernel=16, seq_kernel=59, drop_out=0.2, hidden1 = 192, hidden2 = 96, stream='1s')
    x = torch.randn(1, 192, 875, 25)
    vid_feat = None#torch.randn(1, 12, 97)
    out = seq_model.forward(x,vid_feat)
